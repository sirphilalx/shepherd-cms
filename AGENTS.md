...
- The CMS read layer fetches **published** content through `next-sanity` against a **public dataset** (CDN-served, no token needed — published sermons, library items, announcements, and events are meant for the world). **Draft and in-review content** is only ever fetched through a **server-only, tokened** client, and only for users with the relevant role. Note: give seeded/fixture documents **single-segment ids** (hyphens, not dots) — a dot in an `_id` starts a path segment and a public dataset only grants anonymous read to `_id in path("*")`.
...
The Sanity dataset **may be public** — published CMS content (sermons, library items, announcements, events) is world-readable by design, and drafts/versions stay private on a public dataset regardless (they are excluded from the anonymous-read ACL). Do not use a client-side Sanity **write** token or any client-side Postgres token, an embedded Studio, or a separate backend framework. Do not model attendance, headcount, schedule, or membership data as Sanity documents — see section 13 for why.
